# VolcanoChat 🌋

<div align="center">

**基于火山引擎的 macOS AI 语音助手**

一个功能强大的 macOS 原生应用，集成火山引擎 AI 服务，支持语音对话、智能截图和多模态交互。

[![Swift](https://img.shields.io/badge/Swift-5.9-orange.svg)](https://swift.org)
[![Platform](https://img.shields.io/badge/Platform-macOS%2013.0+-blue.svg)](https://www.apple.com/macos/)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)

[功能特性](#功能特性) • [安装](#安装) • [配置](#配置) • [使用说明](#使用说明) • [技术栈](#技术栈) • [开发](#开发)

</div>

---

## ✨ 功能特性

### 🤖 AI 对话
- **智能回复**：集成火山引擎豆包大模型，默认使用 `doubao-seed-1-6-lite-251015`
- **流式响应**：实时显示 AI 回复，无需等待
- **Markdown 渲染**：支持代码高亮、列表、引用等格式
- **默认优化**：内置简洁回复提示词，AI 回答更精准高效
- **会话管理**：保留对话历史，支持清空会话

### 🎤 语音识别（ASR）
- **实时转文字**：火山引擎 ASR 服务，识别准确率高
- **双模式录音**：
  - 全局录音：按住快捷键随时录音
  - 截图内录音：截图后可继续录音，发送图片+文字
- **可视化反馈**：精美的录音动画（脉冲波 + 音量波形）
- **高质量音频**：16kHz WAV 格式，保证识别质量

### 🔊 语音合成（TTS）
- **自动播报**：AI 回复可自动转语音播放（默认开启）
- **格式清理**：自动移除 Markdown 格式符号，只朗读纯文本
- **流畅体验**：支持多种音色，语音自然流畅

### ⌨️ 全局快捷键
- **录音快捷键**（默认 `⌥ Option`）：
  - 按下开始录音，松开自动识别并发送
  - 精美悬浮窗显示录音状态
  - 三层脉冲动画 + 实时音量波形
- **截图快捷键**（默认 `⌃A Control+A`）：
  - 全屏区域选择
  - 支持涂鸦标注
  - 截图后可录音，发送多模态消息

### 📸 智能截图
- **区域选择**：拖拽选择任意区域
- **涂鸦标注**：支持在截图上自由涂鸦（红色画笔）
- **多模态交互**：
  - 单纯截图：选择区域后松开鼠标，自动保存到剪贴板
  - 截图+语音：选择区域后按住录音键，松开发送图片+文字给 AI
- **高质量**：自动排除应用窗口，支持 Retina 和多显示器
- **快捷操作**：即使应用未激活也能使用

### 🖼️ 图片消息
- **多图上传**：输入框支持粘贴多张图片
- **左对齐显示**：图片和文字在气泡中统一左对齐
- **全屏预览**：点击图片查看大图

### 🌐 联网搜索
- **实时搜索**：AI 可搜索网络获取最新信息
- **灵活配置**：可在设置中开启/关闭（默认关闭）

---

## 📦 安装

### 系统要求
- macOS 13.0 或更高版本
- 火山引擎账号（方舟平台 + 语音服务）

### 从源码编译
```bash
git clone https://github.com/yourusername/VolcanoChat.git
cd VolcanoChat
open VolcanoChat.xcodeproj
```

在 Xcode 中编译并运行（⌘R）

---

## ⚙️ 配置

### 1. 火山引擎方舟平台

访问 [火山引擎方舟平台](https://console.volcengine.com/ark/region:ark+cn-beijing/apiKey)

1. 创建应用并获取 **API Key**
2. 创建推理接入点并获取 **[Model ID](https://console.volcengine.com/ark/region:ark+cn-beijing/model)**

### 2. 语音服务

访问 [火山引擎语音技术控制台](https://console.volcengine.com/speech/service)（切换到旧版，选择音视频字幕生成）

1. 创建项目并获取 **App ID**
2. 生成 **Access Token**

### 3. 应用配置

打开应用后，按 `⌘,` 进入设置：

#### 方舟平台配置
- **API Key**: 粘贴方舟平台的 API Key
- **模型/Endpoint ID**: 粘贴 Model ID（默认使用 `doubao-seed-1-6-lite-251015`）

#### 语音服务配置
- **App ID**: 粘贴语音服务的 App ID
- **Access Token**: 粘贴 Access Token

#### 快捷键设置
- **录音快捷键**: 点击输入框，按下快捷键组合（默认 `⌥ Option`）
- **截图快捷键**: 点击输入框，按下快捷键组合（默认 `⌃A`）

#### 功能开关
- **自动语音播报**: AI 回复自动转语音播放（默认开启）
- **启用联网搜索**: 允许 AI 搜索实时信息（默认关闭，可以在[联网内容插件](https://console.volcengine.com/ark/region:ark+cn-beijing/components)处开启）

#### 系统提示词（可选）
- 留空使用默认提示词：`你会言简意赅的输出内容，不要说没用的废话，只输出必要信息`
- 可自定义

### 4. 权限授予

首次使用时需要授予以下权限：

- **麦克风权限**: 用于语音录制
- **辅助功能权限**: 用于全局快捷键
- **屏幕录制权限**: 用于截图功能

---

## 🚀 使用说明

### 语音对话
1. 按住录音快捷键（默认 `⌥ Option`）
2. 对着麦克风说话（右上角显示录音动画）
3. 松开快捷键，自动识别并发送给 AI
4. AI 回复会显示在对话窗口并可选择语音播报

### 文字对话
1. 在底部输入框输入消息
2. 按 `Enter` 或点击发送按钮
3. AI 回复支持 Markdown 格式

### 截图 + 剪贴板
1. 按下截图快捷键（默认 `⌃A`）
2. 拖拽鼠标选择区域
3. 可选：在选区上涂鸦标注
4. 松开鼠标确认，截图保存到剪贴板

### 截图 + 语音 → AI（多模态）
1. 按下截图快捷键（默认 `⌃A`）
2. 拖拽选择区域
3. 可选：涂鸦标注
4. 按住录音快捷键（默认 `⌥ Option`）
5. 对着麦克风描述问题
6. 松开录音键，图片+文字一起发送给 AI

### 图片消息
1. 在输入框按 `⌘V` 粘贴图片
2. 输入问题文字
3. 发送后 AI 会分析图片内容

---

## 🛠️ 技术栈

### 核心框架
- **SwiftUI** - 现代化 UI 框架
- **AppKit** - macOS 原生控件
- **Combine** - 响应式编程

### 系统框架
- **ScreenCaptureKit** - 屏幕截图（macOS 13+）
- **AVFoundation** - 音频录制和播放
- **Carbon** - 全局键盘事件监听

### 火山引擎集成
- 方舟平台 LLM API（支持流式响应）
- 语音识别 (ASR) API
- 语音合成 (TTS) API

### 架构模式
- **MVVM** - Model-View-ViewModel 架构
- **@MainActor** - Swift 并发安全
- **ObservableObject** - 状态管理

---

## 👨‍💻 开发

### 项目结构
```
VolcanoChat/
├── App/                          # 应用入口
│   └── VolcanoChatbotApp.swift  # 主应用
│
├── Models/                       # 数据模型
│   ├── Message.swift            # 消息模型
│   ├── Conversation.swift       # 会话模型
│   ├── AudioRecorder.swift      # 音频录制器
│   └── AudioPlayer.swift        # 音频播放器
│
├── ViewModels/                   # 视图模型
│   ├── ChatViewModel.swift              # 聊天视图
│   ├── SettingsViewModel.swift          # 设置页面
│   ├── GlobalRecordingManager.swift     # 全局录音管理
│   └── GlobalScreenshotManager.swift    # 全局截图管理
│
├── Views/                        # 视图层
│   ├── ChatView.swift                   # 聊天界面
│   ├── MessageRow.swift                 # 消息气泡
│   ├── InputBar.swift                   # 输入框
│   ├── SettingsView.swift               # 设置界面
│   ├── RecordingFloatingPanel.swift     # 录音悬浮窗
│   ├── ScreenshotOverlayWindow.swift    # 截图选择窗口
│   ├── ImageAttachmentView.swift        # 图片附件
│   └── MarkdownText.swift               # Markdown 渲染
│
├── Services/                     # 服务层
│   ├── VolcanoLLMService.swift          # 大语言模型
│   ├── VolcanoASRService.swift          # 语音识别
│   ├── VolcanoTTSService.swift          # 语音合成
│   └── ConversationManager.swift        # 会话管理
│
└── Utils/                        # 工具类
    ├── GlobalHotkeyManager.swift        # 全局快捷键
    ├── ScreenshotCapture.swift          # 截图捕获
    ├── ClipboardHelper.swift            # 剪贴板工具
    ├── VolcanoConfig.swift              # 配置管理
    └── KeychainHelper.swift             # 钥匙串存储
```

### 构建要求
- Xcode 15.0+
- Swift 5.9+
- macOS 13.0+ Deployment Target

### 核心实现

#### 全局快捷键
使用 Carbon API 监听系统级按键事件：
```swift
// 支持修饰键（Option, Command, Control）+ 可选按键组合
GlobalHotkeyManager.shared.setHotkey(keyCode: keyCode, modifiers: modifiers)
```

#### 截图涂鸦
在截图选择窗口上实时绘制路径：
```swift
// 记录触摸路径点，渲染到最终截图
func renderDrawingsOnImage(_ image: NSImage, drawings: [DrawingPath]) -> NSImage
```

#### 多模态消息
支持图片+文字组合发送：
```swift
await conversationManager.sendMessage(text: recognizedText, images: [imageAttachment])
```

---

## 🙏 致谢

- [火山引擎](https://www.volcengine.com/) - 提供 AI 服务支持
- [Apple](https://www.apple.com/) - 提供 macOS 开发框架

---

## 📮 联系方式

如有问题或建议，欢迎：
- 提交 [Issue](https://github.com/yourusername/VolcanoChat/issues)
- 发起 [Pull Request](https://github.com/yourusername/VolcanoChat/pulls)

---

<div align="center">

**Made with ❤️ using Swift and SwiftUI**

🤖 Generated with [Claude Code](https://claude.com/claude-code)

</div>
